...
PVR_ERROR RecordingBuffer::GetStreamTimes(PVR_STREAM_TIMES *stimes)
{
  ...
  stimes->ptsStart = 0;
  stimes->ptsBegin = 0;
  stimes->ptsEnd = ((int64_t ) RecordingBuffer::Duration() ) * DVD_TIME_BASE;
...
{
  int dataRead = XBMC->ReadFile(m_inputHandle, buffer, length);
  if (dataRead==0 && m_isRecording)
  {
    ...
    {
      long where = XBMC->GetFileLength(m_inputHandle);
